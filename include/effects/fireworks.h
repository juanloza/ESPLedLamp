+--------------------------------------------------------------------------
//
// NightDriver - (c) 2018 Dave's Garage (YouTube Channel 2020)
//
...
public class FireworksEffect : LEDEffect
{
    ...
    protected Palette256 _Palette = new Palette256(CRGB.Rainbow);

    public bool       Blend                     = true;
    public double     MaxSpeed                  = 375.0f;        // Max velocity in pixels per second
    public double     NewParticleProbability    = 0.005f;         // Odds of a new particle each pass
    public double     ParticlePreignitonTime    = 0.0f;          // How long to "wink" into existence
    public double     ParticleIgnition          = 0.2f;          // How long to "flash" white at birth
    public double     ParticleHoldTime          = 0.00f;         // Main lifecycle time
    public double     ParticleFadeTime          = 2.0f;          // Fade out time
    public double     ParticleSize              = 0.00f;         // Size of the particle (0 is min)
    ...
    protected override void Render(ILEDGraphics graphics)
    {
        ...
        for (int iPass = 0; iPass < graphics.DotCount / 50; iPass++)
        {
            if (_random.NextDouble() < NewParticleProbability)
            {
                ...
                CRGB color = _Palette.ColorFromPalette((byte)_random.Next(0, 255), 1.0f, false);
        ...
        foreach (Particle star in _Particles)
        {
            ...
            ParticleSize = (1 - fade) * (graphics.DotCount / 500);
